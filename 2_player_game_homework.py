...
def move_ai(red, yellow, bullets_y, bullets_r):

    speed = 10

    for b in bullets_y:

        if b.x > border.x and red.x < b.x < red.x + 400:

            if b.y < red.y and red.y - speed > 0:
                red.y += speed  
            elif b.y > red.y and red.y + red.height + speed < HEIGHT:
                red.y -= speed  
 
            break

    if yellow.y + yellow.height / 2 < red.y:
        red.y -= speed
    elif yellow.y + yellow.height / 2 > red.y + red.height:
        red.y += speed

    if random.random() < 0.02:
        red.x += random.choice([-speed, speed])
        red.x = max(border.x + border.width + 10, min(WIDTH - red.width - 10, red.x))


    if ((yellow.y + yellow.height/2) - (red.y + red.height/2)) < 40 and random.randint(0,50) < 10:
